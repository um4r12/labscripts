...
for fileName in *.nii;
	do
	folder="$(cut -d'-' -f1 <<< "$fileName")"
	theRest="$(cut -d'-' -f2 <<< "$fileName")"
	timepoint="$(cut -d'_' -f1 <<< "$theRest")"
	mode="$(cut -d'_' -f5 <<< "$theRest")" 
	mode="$(cut -d'.' -f1 <<< "$mode")";
	echo $mode;
	if [ "$timepoint" = 0 ]; then 
		echo "/usr/local/fsl/bin/flirt -in $fileName -applyxfm -init /home/fsluser/Desktop/T1_images/$folder/A_halfwayto_B.mat -out /home/fsluser/Desktop/$folder-$timepoint_halfway_$mode -paddingsize 0.0 -interp trilinear -ref $fileName"
	else
	        echo "/usr/local/fsl/bin/flirt -in $fileName -applyxfm -init /home/fsluser/Desktop/T1_images/$folder/B_halfwayto_A.mat -out /home/fsluser/Desktop/$folder-$timepoint_halfway_$mode -paddingsize 0.0 -interp trilinear -ref $fileName"
	fi
	echo $folder $timepoint; done
